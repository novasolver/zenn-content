...
| 中 | spring-pendulum | preset `applyPreset('resonance')` 約L369 | 「共振」プリセット(k=2,m=0.5,L0=0.5)の周波数比が ω_spring/ω_pendulum=√(k/m)/√(g/L0)=2.0/4.43=**0.45** で、2:1自動パラメトリック共振条件(ω_spring=2ω_pendulum)から大きく外れる。resonanceBox の判定 `|ratio-2|<0.2` も成立せず、きれいなエネルギー授受が見えない。真の2:1には k≈39.2 N/m が必要(m=0.5,L0=0.5)。 | resonance プリセットを 2:1 条件 (k≈39, または m/L0調整) に修正。ラベルと物理を一致させる。 |
